...
2. Generate the security checksums:
...
1. **Terminal:** Simply run `omarchy-gpu-menu`.
    
2. **App Launcher:** Search for "GPU Mode Switcher" in your desktop environment's application menu.
    
3. **Keybind:** Map `omarchy-gpu-menu` to a custom keybind in your Hyprland/Sway config. For example, in `hyprland.conf`:
...
_(Note: If you run the script directly from the downloaded GitHub folder, it will prioritize the local `./config/` directory for easy development and testing)._
